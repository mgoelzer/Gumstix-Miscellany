...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		c = getopt_long (argc, argv, "nsr:d:N:vh", long_options, NULL);
		if (c == -1)
			break;
...
void main_loop(const char *cdevice, int sample_rate)
{
	...
	{	
		...
		for(loop=0; loop < max_bits;)
		{
			...
			if (n_output_bytes > 0)
			{
				int cur_added = add_to_kernel_entropyspool(random_fd, output_buffer, n_output_bytes);
...
void get_random_data(int sample_rate, int skip_samples, int process_samples, int *n_output_bytes, char **output_buffer)
{
	...
	*n_output_bytes=0;
	...
	input_buffer_size = snd_pcm_frames_to_bytes(chandle, max(skip_samples, process_samples));
